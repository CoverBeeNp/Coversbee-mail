...
import { formatAddress, type ParsedOrder } from '@/lib/parser/blanxerParser'

// Shared by the manual-paste flow (app/(staff)/orders/actions.ts) and the
// Blanxer API sync route (app/api/blanxer/sync-orders) — both end up with a
// ParsedOrder, just from different sources (a copy-pasted order page vs. a
// GET /order/:store_id/:order_id response), so the customer-upsert + order-
// insert logic only needs to exist once.
export async function saveParsedOrder(
  supabase: SupabaseClient,
  input: { parsed: ParsedOrder; rawPastedText: string | null; blanxerId?: string }
): Promise<{ orderId: string }> {
  const { parsed, rawPastedText, blanxerId } = input
